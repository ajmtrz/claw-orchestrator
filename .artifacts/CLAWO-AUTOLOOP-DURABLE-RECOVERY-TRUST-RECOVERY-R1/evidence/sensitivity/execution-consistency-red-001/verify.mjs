...
export function containedPath(directory, relative) {
  requireThat(typeof relative === 'string' && relative.length > 0 && !path.isAbsolute(relative) &&
    !relative.includes('\\') && relative.split('/').every((s) => s && s !== '.' && s !== '..'), 'Invalid artifact path');
...
function pointerValue(value, pointer) {
  ...
  for (const escaped of pointer.slice(1).split('/')) {
    ...
    requireThat(value !== null && typeof value === 'object' && Object.hasOwn(value, key), 'Missing observation JSON pointer');
    value = value[key];
  }
  return value;
}
...
const diagnosticFailure = /^(?:#\s*)?(?:Error:.*(?:unhandled|uncaught)|Unhandled(?:Promise)?Rejection|uncaughtException|.*generated asynchronous activity after the test ended)/im;
...
export function inspectTestReport(bytes, format) {
  ...
  if (format === 'node-tap') {
    ...
    const count = (name) => {
      ...
      if (matches.length !== 1) { result.errors.push(`Missing or duplicate TAP ${name} summary`); return -1; }
    ...
    for (const key of ['fail', 'cancelled', 'skipped', 'todo']) if (count(key) !== 0) result.errors.push(`TAP ${key} is nonzero`);
    ...
    if (report.success !== true || report.numFailedTests !== 0 || report.numFailedTestSuites !== 0 ||
      report.numTotalTests !== result.discovered.length || report.numPassedTests !== result.executed.length ||
      report.numPendingTests !== 0 || (report.numTodoTests ?? 0) !== 0 || report.unhandledErrors?.length ||
      (report.numUnhandledErrors ?? 0) !== 0) result.errors.push('Failed or inconsistent Vitest report');
...
function verifyExecution(execution, directory, expected) {
  ...
  requireThat(execution.exit_code === 0 && execution.signal === null && !execution.timed_out, 'Failed execution exit, signal or timeout');
  const start = Date.parse(execution.started_at), end = Date.parse(execution.ended_at);
  ...
  requireThat(!diagnosticFailure.test(stderr.toString('utf8')) && !diagnosticFailure.test(stdout.toString('utf8')), 'Unhandled execution diagnostic');
  ...
  for (const id of expected.required_test_ids) requireThat(report.executed.includes(id), `Missing required test: ${id}`);
}
...
function applyPredicate(assertion, values, observations) {
  ...
  switch (assertion.predicate) {
    case 'equal':
      valid = args.length === 2 && !args.some((v) => typeof v === 'boolean') && isDeepStrictEqual(args[0], args[1]); break;
    case 'literal':
      requireThat(Object.hasOwn(assertion, 'expected') && typeof assertion.expected !== 'boolean', 'Boolean or missing literal predicate');
      valid = args.length === 1 && isDeepStrictEqual(args[0], assertion.expected); break;
    case 'digest-equal':
      valid = args.length === 2 && args.every((v) => typeof v === 'string') && sha256(args[0]) === args[1]; break;
    case 'ordered': {
      const events = assertion.observations.map((id) => observations.get(id));
      valid = events.length > 1 && events.every((e, i) => i === 0 ||
        (e.execution_id === events[0].execution_id && e.observer_id === events[0].observer_id && e.sequence > events[i - 1].sequence));
      break;
    }
    case 'row-count':
      valid = args.length === 1 && Array.isArray(args[0]) && Number.isSafeInteger(assertion.expected) && assertion.expected >= 0 && args[0].length === assertion.expected; break;
    case 'prefix-preserved':
      valid = args.length === 2 && args.every((v) => typeof v === 'string') && Buffer.from(args[1]).subarray(0, Buffer.byteLength(args[0])).equals(Buffer.from(args[0])); break;
    default: throw new Error('Unsupported predicate');
  }
...
export function verifyBundle(bundle, directory, contract) {
  ...
  for (const key of ['run_id', 'case_id', 'subject_kind', 'base', 'head', 'tree', 'requirement_ids',
    'harness_sha256', 'dependency_sha256', 'tool_sha256']) same(bundle[key], contract[key], `Wrong ${key}`);
  ...
  for (const entries of Object.values(manifest)) requireThat(!duplicates(entries.map((e) => e.path)), 'Duplicate manifest path');
  for (const [field, key] of [['harness_sha256', 'harness'], ['dependency_sha256', 'dependencies'], ['tool_sha256', 'tools']])
    same(bundle[field], sha256(JSON.stringify(manifest[key])), `Manifest ${field} mismatch`);
  if (!bundle.frozen) requireThat(bundle.patch !== null && bundle.test_sources.length > 0, 'Unfrozen evidence requires exact patch and test bytes');
  ...
  const values = new Map(), observations = new Map(), sequences = new Map();
  for (const observation of bundle.observations) {
    ...
    for (const field of ['sequence', 'process_id', 'observer_id', 'value']) same(event?.[field], observation[field], `Observation ${field} differs from artifact`);
    const stream = JSON.stringify([observation.execution_id, observation.observer_id]);
    requireThat(observation.sequence > (sequences.get(stream) ?? -1), 'Observation sequence/order is not strictly increasing');
    sequences.set(stream, observation.sequence);
    values.set(observation.id, event.value);
    observations.set(observation.id, observation);
  }
  for (const id of [...contract.required_observations, ...contract.required_fault_observations]) requireThat(values.has(id), `Missing required/fault observation: ${id}`);
  ...
  for (const source of bundle.source_imports) {
    ...
    requireThat(receipt?.path === source.path && receipt?.sha256 === source.sha256, 'Source import differs from observed import receipt');
  ...
  for (const sourcePath of contract.required_source_paths) requireThat(bundle.source_imports.some((s) => s.path === sourcePath), 'Missing required source import');
...
export function verifySeries(entries, contract, repetitions) {
  requireThat(Number.isSafeInteger(repetitions) && repetitions > 0 && entries.length === repetitions, 'Incomplete repetitions');
  const seen = new Set();
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    requireThat(mode === 'preparation' && directory && extra.length === 0,
