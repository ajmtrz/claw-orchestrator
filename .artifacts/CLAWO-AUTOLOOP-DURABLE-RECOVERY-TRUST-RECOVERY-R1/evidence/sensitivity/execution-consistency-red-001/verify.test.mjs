...
if (process.env.CLAWO_TRUST_CAPTURE_ID) {
  for (const [index, url] of [import.meta.url, verifySource, collectorSource].entries()) {
    ...
    console.log('TRUST_RECOVERY_OBSERVATION ' + JSON.stringify({
      sequence: index, process_id: process.pid, observer_id: `node-test:${process.pid}`,
...
function fixture() {
  ...
  const contract = {
    run_id: run, case_id: 'validator-fixture', subject_kind: 'sensitivity',
    base: sha, head: sha, tree: sha, frozen: false,
    ...
    requirement_ids: ['HARNESS-VALIDATION'],
    executions: [{ argv: ['node', '--test', 'fixture.test.mjs'], cwd: '/fixture', required_test_ids: ['fixture command'] }],
...
for (const [name, mutate, error] of [
  ['fabricated invariant booleans', (f) => { f.bundle.invariants = { exactly_once: true }; }, /schema|additional/i],
  ['missing artifacts', (f) => { fs.unlinkSync(path.join(f.directory, 'events.json')); }, /artifact|ENOENT/i],
  ['changed artifact hashes', (f) => { fs.appendFileSync(path.join(f.directory, 'events.json'), '\n'); }, /hash/i],
  ['wrong subject kind', (f) => { f.bundle.subject_kind = 'candidate'; }, /subject/i],
  ['wrong candidate head', (f) => { f.bundle.head = 'b'.repeat(40); }, /head/i],
  ['wrong input manifest', (f) => { f.contract.input_manifest_sha256 = 'b'.repeat(64); }, /manifest/i],
  ['hidden process failure', (f) => { f.bundle.executions[0].exit_code = 1; }, /execution|exit/i],
  ['a recorded spawn error with exit zero', (f) => { f.bundle.executions[0].spawn_error = 'ENOENT'; }, /execution|spawn/i],
  ['success without an observed process', (f) => { f.bundle.executions[0].process_id = null; }, /execution|process/i],
  ['a terminating signal', (f) => { f.bundle.executions[0].exit_code = null; f.bundle.executions[0].signal = 'SIGKILL'; }, /execution|signal/i],
  ['a timeout', (f) => { f.bundle.executions[0].timed_out = true; }, /execution|timeout/i],
  ['zero selected tests', (f) => { f.bundle.executions[0].executed_test_ids = []; }, /test/i],
  ['required skips', (f) => { f.bundle.executions[0].skipped_test_ids = ['fixture command']; }, /skip|test/i],
  ['fabricated parsed values', (f) => { f.bundle.observations[0].value = 'directive B\n'; }, /value/i],
  ['reordered observation declarations', (f) => { f.bundle.observations.reverse(); }, /order|sequence/i],
  ['fabricated event sequence', (f) => { f.bundle.observations[0].sequence = 20; }, /sequence/i],
  ['a missing fault observation', (f) => { f.contract.required_fault_observations = ['process-loss']; }, /fault|observation/i],
  ['removed predicates', (f) => { f.bundle.assertions = []; }, /assertion|schema/i],
  ['weakened row-count predicate', (f) => { f.bundle.assertions[2].expected = 0; }, /assertion|contract/i],
  ['Coder-authored review claims', (f) => { f.bundle.review = { author: 'coder', decision: 'advance' }; }, /schema|additional/i],
  ['unrecorded dirty test bytes', (f) => { f.bundle.test_sources = []; }, /patch|test|unfrozen/i],
  ['missing RED patch', (f) => { f.bundle.patch = null; }, /patch|unfrozen/i],
  ['an unfrozen snapshot relabeled as frozen', (f) => { f.bundle.frozen = true; f.bundle.patch = null; }, /frozen/i],
  ['substituted RED patch bytes', (f) => { f.bundle.patch = f.artifact('wrong.patch', 'different patch\n'); }, /patch/i],
  ['substituted test source bytes', (f) => { f.bundle.test_sources = [f.artifact('wrong-test.txt', 'different test\n')]; }, /test.*source/i],
  ['incorrect command identity', (f) => { f.bundle.executions[0].argv = ['node', '-e', 'process.exit(0)']; }, /command|argv/i],
  ['duplicate execution identity', (f) => { f.bundle.executions.push(structuredClone(f.bundle.executions[0])); }, /execution|duplicate/i],
  ['unknown observation execution', (f) => { f.bundle.observations[0].execution_id = 'invented'; }, /execution/i],
  ['fabricated observer identity', (f) => { f.bundle.observations[0].observer_id = 'invented'; }, /observer/i],
  ['out-of-run path traversal', (f) => { f.bundle.observations[0].artifact.path = '../events.json'; }, /path|artifact/i],
  ['absolute artifact paths', (f) => { f.bundle.observations[0].artifact.path = path.join(f.directory, 'events.json'); }, /path|artifact/i],
]) {
  test(`rejects ${name}`, () => {
    const f = fixture();
    mutate(f);
...
test('collector retains real child stdout, stderr, argv, cwd, exit and test inventory', async () => {
  const f = fixture();
  const argv = [process.execPath, '-e', `process.stdout.write(${JSON.stringify(tap)}); process.stderr.write('diagnostic\\n'); process.exitCode = 7;`];
  const execution = await captureExecution({ directory: f.directory, id: 'child-capture', argv, cwd: f.directory, timeout_ms: 3000 });
...
test('collector kills its timed-out process and retains its terminating signal', async () => {
  const f = fixture();
  const execution = await captureExecution({ directory: f.directory, id: 'timeout-capture', argv: [process.execPath, '-e', 'setInterval(() => {}, 1000)'], cwd: f.directory, timeout_ms: 100 });
...
test('collector preserves spawn failure rather than inventing a successful process', async () => {
  const f = fixture();
  const execution = await captureExecution({ directory: f.directory, id: 'spawn-failure', argv: [path.join(f.directory, 'missing-command')], cwd: f.directory, timeout_ms: 1000 });
...
test('collector redacts known credential forms and records the applied rules', async () => {
  ...
  const execution = await captureExecution({ directory: f.directory, id: 'redaction-capture', argv: [process.execPath, '-e', `process.stdout.write('Authorization: Bearer ${secret}\\n'); process.stderr.write('OPENAI_API_KEY=${secret}\\n')`], cwd: f.directory, timeout_ms: 3000 });
...
test('collector refuses overwrites and output paths outside the run artifact root before spawning', async () => {
  ...
  for (const directory of [path.join(project, 'tasks', run), path.dirname(scratch)]) {
    await assert.rejects(captureExecution({ directory, id: 'protected-path', argv, cwd: project, timeout_ms: 1000 }), /path|directory/i);
  }
  fs.writeFileSync(path.join(f.directory, 'existing.stdout.txt'), 'preserved');
  await assert.rejects(captureExecution({ directory: f.directory, id: 'existing', argv, cwd: project, timeout_ms: 1000 }), /exist|overwrite/i);
...
test('collector cannot use a symlink to redirect output into a protected directory', async () => {
  ...
  await assert.rejects(captureExecution({ directory: path.join(f.directory, 'redirect'), id: 'escape', argv: [process.execPath, '-e', 'process.exit(0)'], cwd: project, timeout_ms: 1000 }), /symlink/i);
});
...
test('validates literal, equality and byte-preserving prefix predicates against event values', () => {
  for (const [predicate, left, right, expected, changed] of [
    ...
    const f = fixture();
    f.events[0].value = left; f.events[1].value = right;
    const replaceEvents = (name) => {
      const ref = f.artifact(name, JSON.stringify(f.events));
      for (const [i, o] of f.bundle.observations.entries()) { o.artifact = ref; o.value = f.events[i].value; }
    };
    replaceEvents('predicate-events.json');
    const assertion = { id: 'contract', predicate, observations: predicate === 'literal' ? ['payload'] : ['payload', 'digest'] };
    if (expected !== undefined) assertion.expected = expected;
    f.bundle.assertions = [assertion]; f.contract.assertions = structuredClone(f.bundle.assertions);
...
test('validates Vitest inventory and rejects hidden failures even when success remains true', () => {
  const base = { success: true, numTotalTests: 1, numPassedTests: 1, numFailedTests: 0, numFailedTestSuites: 0,
    numPendingTests: 0, numTodoTests: 0,
    testResults: [{ name: '/fixture/test.ts', status: 'passed', message: '', assertionResults: [{ fullName: 'a case', status: 'passed', failureMessages: [] }] }] };
  for (const mutation of [null, (r) => { r.numFailedTests = 1; }, (r) => { r.numPendingTests = 1; },
    (r) => { r.unhandledErrors = ['unhandled rejection']; }, (r) => { r.testResults[0].assertionResults[0].failureMessages = ['hidden failure']; }]) {
    const f = fixture();
...
test('a green Vitest report cannot hide a failing test in raw stdout', () => {
  const f = fixture();
  const report = { success: true, numTotalTests: 1, numPassedTests: 1, numFailedTests: 0, numFailedTestSuites: 0, numPendingTests: 0,
    testResults: [{ name: '/fixture/test.ts', status: 'passed', message: '', assertionResults: [{ fullName: 'a case', status: 'passed', failureMessages: [] }] }] };
  const e = f.bundle.executions[0];
  e.report_format = 'vitest-json'; e.report = f.artifact('vitest.json', JSON.stringify(report));
